...
app.post("/notice", async (req, res) => {
  ...
  const config = {
    headers: {
      Authorization: "Bearer EAAJXRz8WahsBO7m1BxDz4ZBUrEtvh1oUz7QvaHkOVb7aDA1iBLVifYQwpzAbHrDBxZBymsNdQYrhtoves8ZAR8WHI1ZAOg88BRFDsqLxLHpDWMo76w9y6adqFbl9uxKnPgG1RZBya3JVCR3VQxEafiLUSBPjjZBUZBaFkGUQLtJY80I110tZC47R3b8u2i4ici2hgVc0Q0F4meRkZAzYw",
      "Content-Type": "application/json",
    },
  };

  try {
    const dataNoticeGuardian = {
      messaging_product: "whatsapp",
      to: "541122520510",
      type: "template",
      template: {
        name: "aviso_compra_guardianes",
        language: {
          code: "es_AR",
        },
        components: [
    ...
    res.status(200).json("Guardian notificado");
  } catch (error) {
    console.log(error);
